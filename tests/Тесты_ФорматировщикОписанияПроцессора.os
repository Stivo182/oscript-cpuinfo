...
&Тест
Процедура ТестДолжен_ФормироватьПолноеОписаниеПроцессора() Экспорт
...
&Тест
Процедура ТестДолжен_ФормироватьКраткиеОписанияIntelCore() Экспорт
...
&Тест
Процедура ТестДолжен_ФормироватьОписанияIntelСРазнымиЧастотами() Экспорт
...
&Тест
Процедура ТестДолжен_ФормироватьОписанияAmd() Экспорт

	// Подготовка
	ТестовыеСлучаи = Новый Массив();
	ФорматировщикОписания = Новый ФорматировщикОписанияПроцессора();
	...
	Для Каждого ТестовыйСлучай Из ТестовыеСлучаи Цикл
		...
		Ожидаем
			.Что(ФорматировщикОписания.КраткоеОписание(ИнформацияОПроцессоре, Истина))
...
&Тест
Процедура ТестДолжен_ФормироватьОписаниеПослеИзмененияХарактеристик() Экспорт

	// Подготовка
	ИнформацияОПроцессоре = Новый ИнформацияОПроцессоре(Ложь);
	ИнформацияОПроцессоре.ИмяПроцессора = "Intel(R) Pentium(TM) G4560 CPU @ 3.50GHz";
	ИнформацияОПроцессоре.КраткоеОписание();
	ИнформацияОПроцессоре.ИмяПроцессора = "AMD Ryzen 7 2700X Eight-Core Processor";

	// Действие
	ПроверяемоеЗначение = ИнформацияОПроцессоре.КраткоеОписание();

	// Проверка
	Ожидаем.Что(ПроверяемоеЗначение).Равно("AMD Ryzen 7 2700X Eight-Core Processor");
